...
namespace ChessChallenge.Application
{
    public static class MenuUI
    {
        public static void DrawButtons(ChallengeController controller)
        {
            ...
            if (NextButtonInRow("MyBot vs 500 ELO", ref buttonPos, spacing, buttonSize))
            {
                controller.StartNewBotMatch(ChallengeController.PlayerType.MyBot, ChallengeController.PlayerType.NegamaxBasic);
            }
            if (NextButtonInRow("MyBot vs 1950 ELO", ref buttonPos, spacing, buttonSize))
            {
                controller.StartNewBotMatch(ChallengeController.PlayerType.MyBot, ChallengeController.PlayerType.NegamaxAdvanced);
            }


            // Page buttons
            buttonPos.Y += breakSpacing;
            buttonPos = UIHelper.Scale(new Vector2(405, 210)); buttonSize = UIHelper.Scale(new Vector2(220, 55));

            if (NextButtonInRow("Fast forward", ref buttonPos, spacing, buttonSize))
            {
                controller.fastForward = !controller.fastForward;
                Console.WriteLine(controller.fastForward.ToString());
            }
